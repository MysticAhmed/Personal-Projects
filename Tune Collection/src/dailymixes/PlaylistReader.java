...
public class PlaylistReader
{
        ...
        FileNotFoundException
    {
        ...
        PlaylistWindow window = new PlaylistWindow(playlistCalculator);
        ...
        FileNotFoundException
    {
            ...
            && playlistCounter < PlaylistCalculator.NUM_PLAYLISTS)
        {
            ...
            if (tokens.length < 8)
            {
                ...
                throw new ParseException(
                    "There are less than 8 comma seperated values on the line",
                    tokens.length);
            }
            else if (tokens.length > 8)
            {
                ...
                throw new ParseException(
                    "There are more than 8 comma seperated values on the line",
                    tokens.length);
            }
            ...
            else
            {
                ...
                if (!this.isInValidPercentRange(minPop, minRock, minCountry)
                    || !this.isInValidPercentRange(maxPop, maxRock, maxCountry))
                {
        ...
        if (playlistCounter < PlaylistCalculator.NUM_PLAYLISTS)
        {
            ...
            throw new DailyMixDataException(
                "Less than required amount of playlists");
        }
        ...
        FileNotFoundException
    {
        ...
        while (file.hasNextLine())
        {
                ...
                && tokens.length != SONG_TOKENS - 1)
            {
                ...
                throw new ParseException(
                    "Invalid number of tokens " + tokens.length,
                    0);
            }
            ...
            if (!isInValidPercentRange(pop, rock, country))
            {
                ...
                throw new DailyMixDataException(
                    "Invalid percentages for song: " + songName);
            }
            ...
            {
                playlist = tokens[4];
                newSong = new Song(songName, pop, rock, country, playlist);
            }
            else
            {
                newSong = new Song(songName, pop, rock, country, playlist);
